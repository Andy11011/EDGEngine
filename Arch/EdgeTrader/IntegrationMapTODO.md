...
### Step 6 – Promote `EVENT_DRIVEN` to Default (Soft Switch)

- [X] ~~*Flip default: `trade_source_mode = os.getenv("TRADE_SOURCE_MODE", "EVENT_DRIVEN").upper()`*~~ [2026-08-06]
- [X] ~~*Keep `SINGLE` branch and `BlueprintStrategy` fully intact as explicit opt-out*~~ [2026-08-06]

### Step 7 – Remove the Legacy Single-Strategy Path (The Big Cleanup)

- [X] ~~*Delete `BlueprintConfig`, `BlueprintStrategy`*~~ [2026-08-06]
- [X] ~~*Delete sync `run_node()`*~~ [2026-08-06]
- [X] ~~*Delete `TRADE_SOURCE_MODE` and the `SINGLE` branch in `main()`*~~ [2026-08-06]
- [X] ~~*`main()` unconditionally: build node → register factories → `node.build()` → `node.run_async()` concurrently with `listen_trade_events(node, sqs_client, queue_url)`*~~ [2026-08-06]
- [X] ~~*Update/remove single-instrument `bar_type` construction in `main()` if no longer needed globally*~~ [2026-08-06]
